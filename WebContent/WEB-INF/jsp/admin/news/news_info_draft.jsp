...
<script type="text/javascript">
	...
	$(function() {
		...
		tab = $('#dataTable')
				.datagrid(
						{
							url : ctx
									+ "/admin/news/manageNewsInfo/query_news_info_draft", //数据来源 
							...
							columns : [ [
									...
									},
									{
										...
										formatter : function(value, row, index) {
											...
											return "<a style='text-decoration:underline;cursor:pointer;' onclick='modify_u("
													+ row.newsId
													+ ");'>修改</a>||"
													+ "<a style='text-decoration:underline;cursor:pointer;' onclick='preview_u("
													+ row.newsId
													+ ");'>查看</a>||"
													+ "<a style='text-decoration:underline;cursor:pointer;' onclick='approval_u("
													+ row.newsId
													+ ");'>提交草稿</a>";
							...
							toolbar : [
									{
										...
										handler : function() {
											openWin(
													"新增",
													ctx
															+ "/admin/news/manageNewsInfo/news_info_draft_detail",
													800, 500);
										}
									},
									'-',
	...
	//修改
	function modify_u(id) {
		openWin("修改",
				"${ctx}/admin/news/manageNewsInfo/news_info_draft_detail?infoId="
						+ id, 800, 500);
	}
	...
	//新闻审批
	function approval_u(id) {
		$.messager.confirm('确认', '您是否要提交审核该新闻？', function(r) {
			if (r) {
				$.ajax({
					url : '${ctx}/admin/news/manageNewsInfo/approveNewsDraft',
					method : 'POST',
					data : {
						newsId : id
					},
