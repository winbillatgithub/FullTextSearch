...
<link rel="stylesheet" href="${ctx}/css/portal/search/style-search-paging-content.css" type="text/css" />
...
window.onload = function(){
	$("#input-cause").keyup(function(){
		getList('cause', 'input-causeId', 'input-cause');
	}).blur(function(){
		onblurEvent('cause', 'input-causeId', 'input-cause');
	}).focus(function(){
		getList('cause', 'input-causeId', 'input-cause');
	});
};
...
<script type="text/javascript">
	...
	function initPagination() {
		...
		link = addPara(link, 'cause', encodeChineseURI(cause));
...
//		return addPara('articleId', articleID);
	...
	function clickSearchCause() {
		var cause= $("#input-cause").val();
		if (cause == null || cause == '') {
			return false;
		}
		currentFullUrl = window.location.href;
		currentFullUrl = removePara(currentFullUrl, 'pageId');
		currentFullUrl = addPara(currentFullUrl, 'pageId', '0');
		currentFullUrl = removePara(currentFullUrl, 'cause');
		currentFullUrl = addPara(currentFullUrl, 'cause', encodeChineseURI(cause));
		window.location.href = currentFullUrl;
		return true;
	}
	...
	function clickSearchButton() {
		...
		var cause = $("#cause").val();
		if (cause == null || '' == cause) {
			$("#cause").removeAttr('name');
		}
		var guideCase = $("#guideCase").val();
		if (guideCase == null || '' == guideCase) {
			$("#guideCase").removeAttr('name');
		}
		var articleType = $("#articleType").val();
		if (articleType == null || '' == articleType) {
			$("#articleType").removeAttr('name');
		}
		var articleCourt = $("#articleCourt").val();
		if (articleCourt == null || '' == articleCourt) {
			$("#articleCourt").removeAttr('name');
		}
		var judgeYear = $("#judgeYear").val();
		if (judgeYear == null || '' == judgeYear) {
			$("#judgeYear").removeAttr('name');
		}
		var label = $("#label").val();
		if (label == null || '' == label) {
	...
	<div id="wap-container" class="wap-container">
		...
		<div id="main-container" class="main-container">
			<h1 id="main-page-header-title">
				...
				<c:if test="${guideCase == null || '' == guideCase || 'false' == guideCase}">
					<fmt:message key="search.cause.h1.FileSearchTitle" />
				</c:if>
			</h1>
			<div id="main-search-container">
				<form method="get" id="main-search-file-form"
					action="${ctx}/cause.html">
					...
					<input id="cause" name="cause" type="hidden" value="${cause}" />
		...
		<div id="search-keyword-container">
			<div id="search-keyword-content-container">
				...
				<span class="searchItem"> 
					...
					<c:if test="${cause != null && '' != cause}">
						<a href="javascript:void(0)" onclick=""> ${cause} <i
							data-control="cause" class="closeIco"></i>
						</a>
					</c:if>
		...
		<div id="search-casecause-container">
			<div id="search-casecause-content-container">
				<!--<fmt:message key="search.cause.div_text.Cause" />-->
				<span class="inputItem">
					<input id="input-causeId" name="input-causeId" value="" style="display: none;" />
					<input id="input-cause" type="text" class="inp-txt w230"
						placeholder="<fmt:message key="search.cause.input.PlaceHolder" />"
						maxlength="45" />
					<input type="submit"
						id="cause-search-button"
						onclick="clickSearchCause()"
						value="<fmt:message key="search.input.SearchButton" />" />
					<div id="input-causeError" style="color: red;"></div>
