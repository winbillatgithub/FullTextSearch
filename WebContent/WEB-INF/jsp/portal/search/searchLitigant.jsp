...
window.onload = function(){
	$("#input-litigant").keyup(function(){
		var litigantType = $("#litigantSelect").val();
		getList(litigantType, 'input-litigantId', 'input-litigant');
	}).blur(function(){
		var litigantType = $("#litigantSelect").val();
		onblurEvent(litigantType, 'input-litigantId', 'input-litigant');
	}).focus(function(){
		var litigantType = $("#litigantSelect").val();
		getList(litigantType, 'input-litigantId', 'input-litigant');
	});
...
<script type="text/javascript">
	...
	function initPagination() {
		...
		var litigantType = 'litigant';
		var articleCourt = $("#articleCourt").val();
		var articleType = $("#articleType").val();
		var judgeYear = $("#judgeYear").val();
		var litigant = $("#litigant").val(); if (litigant != null && '' != litigant) litigantType = 'litigant';
		var executor = $("#executor").val(); if (executor != null && '' != executor) litigantType = 'executor';
		var executed = $("#executed").val(); if (executed != null && '' != executed) litigantType = 'executed';
		var thirdpart = $("#thirdpart").val(); if (thirdpart != null && '' != thirdpart) litigantType = 'thirdpart';
		var lawyer = $("#lawyer").val(); if (lawyer != null && '' != lawyer) litigantType = 'lawyer';
		var lawfirm = $("#lawfirm").val(); if (lawfirm != null && '' != lawfirm) litigantType = 'lawfirm';
		var legalPerson = $("#legalPerson").val(); if (legalPerson != null && '' != legalPerson) litigantType = 'legalPerson';
		var label= $("#label").val();
		var tag= $("#tag").val();
		// 拼接URI
		// 
		...
		link = addPara(link, 'litigant', encodeChineseURI(litigant));
		link = addPara(link, 'executor', encodeChineseURI(executor));
		link = addPara(link, 'executed', encodeChineseURI(executed));
		link = addPara(link, 'thirdpart', encodeChineseURI(thirdpart));
		link = addPara(link, 'lawyer', encodeChineseURI(lawyer));
		link = addPara(link, 'lawfirm', encodeChineseURI(lawfirm));
		link = addPara(link, 'legalPerson', encodeChineseURI(legalPerson));

		link = addPara(link, 'articleType', encodeChineseURI(articleType));
		link = addPara(link, 'articleCourt', encodeChineseURI(articleCourt));
		...
		// Initialize combobox
		$("#litigantSelect").val(litigantType);
...
//		return addPara('articleId', articleID);
	...
	function clickSearchLitigant() {
		var litigantValue= $("#input-litigant").val();
		if (litigantValue == null || litigantValue == '') {
			return false;
		}
		//
		// var litigantType = $("#litigantSelect").combobox('getValue');
		var litigantType = $("#litigantSelect").val();
/*		if (litigantType == 'litigant') {
		} else if (litigantType == 'executor') {
		} else if (litigantType == 'executed') {
		} else if (litigantType == 'thirdpart') {
		} else if (litigantType == 'lawyer') {
		} else if (litigantType == 'lawfirm') {
		} else if (litigantType == 'legalPerson') {
		}
*/		currentFullUrl = window.location.href;
		currentFullUrl = removePara(currentFullUrl, 'pageId');
		currentFullUrl = addPara(currentFullUrl, 'pageId', '0');
		currentFullUrl = removePara(currentFullUrl, 'litigant');
		currentFullUrl = removePara(currentFullUrl, 'executor');
		currentFullUrl = removePara(currentFullUrl, 'executed');
		currentFullUrl = removePara(currentFullUrl, 'thirdpart');
		currentFullUrl = removePara(currentFullUrl, 'lawyer');
		currentFullUrl = removePara(currentFullUrl, 'lawfirm');
		currentFullUrl = removePara(currentFullUrl, 'legalPerson');
		currentFullUrl = addPara(currentFullUrl, litigantType, encodeChineseURI(litigantValue));
		window.location.href = currentFullUrl;
		return true;
	}
	...
	function clickSearchButton() {
		...
		removeName('executor');
		removeName('executed');
		removeName('thirdpart');
		removeName('lawyer');
		removeName('lawfirm');
		removeName('legalPerson');
	...
	<div id="wap-container" class="wap-container">
		...
		<div id="main-container" class="main-container">
			<h1 id="main-page-header-title">
				...
				<c:if test="${guideCase == null || '' == guideCase || 'false' == guideCase}">
					<fmt:message key="search.litigant.h1.FileSearchTitle" />
				</c:if>
			</h1>
			<div id="main-search-container">
				<form method="get" id="main-search-file-form"
					action="${ctx}/litigant.html">
					...
					<input id="litigant" name="litigant" type="hidden" value="${litigant}" />
					<input id="executor" name="executor" type="hidden" value="${executor}" />
					<input id="executed" name="executed" type="hidden" value="${executed}" />
					<input id="thirdpart" name="thirdpart" type="hidden" value="${thirdpart}" />
					<input id="lawyer" name="lawyer" type="hidden" value="${lawyer}" />
					<input id="lawfirm" name="lawfirm" type="hidden" value="${lawfirm}" />
					<input id="legalPerson" name="legalPerson" type="hidden" value="${legalPerson}" />
		...
		<div id="search-keyword-container">
			<div id="search-keyword-content-container">
				...
				<span class="searchItem"> 
					...
					<c:if test="${litigant != null && '' != litigant}">
						<a href="javascript:void(0)" onclick=""> ${litigant} <i
							data-control="litigant" class="closeIco"></i>
						</a>
					</c:if>
					<c:if test="${executor != null && '' != executor}">
						<a href="javascript:void(0)" onclick=""> ${executor} <i
							data-control="executor" class="closeIco"></i>
						</a>
					</c:if>
					<c:if test="${executed != null && '' != executed}">
						<a href="javascript:void(0)" onclick=""> ${executed} <i
							data-control="executed" class="closeIco"></i>
						</a>
					</c:if>
					<c:if test="${thirdpart != null && '' != thirdpart}">
						<a href="javascript:void(0)" onclick=""> ${thirdpart} <i
							data-control="thirdpart" class="closeIco"></i>
						</a>
					</c:if>
					<c:if test="${lawyer != null && '' != lawyer}">
						<a href="javascript:void(0)" onclick=""> ${lawyer} <i
							data-control="lawyer" class="closeIco"></i>
						</a>
					</c:if>
					<c:if test="${lawfirm != null && '' != lawfirm}">
						<a href="javascript:void(0)" onclick=""> ${lawfirm} <i
							data-control="lawfirm" class="closeIco"></i>
						</a>
					</c:if>
					<c:if test="${legalPerson != null && '' != legalPerson}">
						<a href="javascript:void(0)" onclick=""> ${legalPerson} <i
							data-control="legalPerson" class="closeIco"></i>
						</a>
					</c:if>
		...
		<div id="search-casecause-container">
			<div id="search-casecause-content-container">
				<!--<fmt:message key="search.litigant.div_text.Litigant" />-->
				<span class="inputItem">
					 <select id="litigantSelect" style="width: 120px; height: 22px;" >
						<option value="litigant"><fmt:message key="search.litigant.select.litigant" /></option>
						<option value="executor"><fmt:message key="search.litigant.select.executor" /></option>
						<option value="executed"><fmt:message key="search.litigant.select.executed" /></option>
						<option value="thirdpart"><fmt:message key="search.litigant.select.thirdpart" /></option>
						<option value="lawyer"><fmt:message key="search.litigant.select.lawyer" /></option>
						<option value="lawfirm"><fmt:message key="search.litigant.select.lawfirm" /></option>
						<option value="legalPerson"><fmt:message key="search.litigant.select.legalPerson" /></option>
					</select>
					<input id="input-litigantId" name="input-litigantId" value="" style="display: none;" />
					<input id="input-litigant" type="text" name="input-litigant"
						placeholder="<fmt:message key="search.litigant.input.PlaceHolder" />"
						maxlength="45" />
					<input type="submit"
						id="cause-search-button"
						onclick="clickSearchLitigant()"
						value="<fmt:message key="search.input.SearchButton" />" />
					<div id="input-litigantError" style="color: red;"></div>
